...
describe("StringAssertion.toBeLongerThanOrEquals()", () => {
    test("With Valid Data", () => {
        ...
        for (const { value, length } of values) {
            expect(() => assert.string(value).toBeLongerThanOrEquals(length)).not.toThrow();
        }
    });
    ...
    test("With Invalid Data", () => {
        ...
        for (const { value, length } of values) {
            expect(() => assert.string(value).toBeLongerThanOrEquals(length)).toThrow();
        }
    });
});
