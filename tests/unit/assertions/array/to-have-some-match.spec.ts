...
describe("StringAssertion.toHaveSomeMatch()", () => {
    test("With Valid Data", () => {
        ...
        for (const { value, condition } of values) {
            expect(() => assert.array(value as Value).toHaveSomeMatch(condition as Condition)).not.toThrow();
        }
    });
    ...
    test("With Invalid Data", () => {
        ...
        for (const { value, condition } of values) {
            expect(() => assert.array(value as Value).toHaveSomeMatch(condition as Condition)).toThrow();
        }
    });
});
