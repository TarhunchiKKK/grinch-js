...
describe("ArrayAssertion.toHaveLengthBetween()", () => {
    test("With Valid Data", () => {
        ...
        for (const { value, start, end } of values) {
            expect(() => assert.array(value).toHaveLengthBetween(start, end)).not.toThrow();
        }
    });
    ...
    test("With Invalid Data", () => {
        ...
        for (const { value, start, end } of values) {
            expect(() => assert.array(value).toHaveLengthBetween(start, end)).toThrow();
        }
    });
});
