...
describe("ArrayAssertion.toBeShorterThanOrEquals()", () => {
    test("With Valid Data", () => {
        ...
        for (const { value, length } of values) {
            expect(() => assert.array(value).toBeShorterThanOrEquals(length)).not.toThrow();
        }
    });
    ...
    test("With Invalid Data", () => {
        ...
        for (const { value, length } of values) {
            expect(() => assert.array(value).toBeShorterThanOrEquals(length)).toThrow();
        }
    });
});
