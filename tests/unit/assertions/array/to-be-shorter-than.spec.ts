...
describe("ArrayAssertion.toBeShorterThan()", () => {
    test("With Valid Data", () => {
        ...
        for (const { value, length } of values) {
            expect(() => assert.array(value).toBeShorterThan(length)).not.toThrow();
        }
    });
    ...
    test("With Invalid Data", () => {
        ...
        for (const { value, length } of values) {
            expect(() => assert.array(value).toBeShorterThan(length)).toThrow();
        }
    });
});
