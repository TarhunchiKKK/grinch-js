...
describe("NumberAssertion.toBeLessThan()", () => {
    test("With Valid Data", () => {
        ...
        for (const { value, argument } of values) {
            expect(() => assert.number(value).toBeLessThan(argument)).not.toThrow();
        }
    });
    ...
    test("With Invalid Data", () => {
        ...
        for (const { value, argument } of values) {
            expect(() => assert.number(value).toBeLessThan(argument)).toThrow();
        }
    });
});
