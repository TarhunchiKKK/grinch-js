...
describe("NumberAssertion.toBeGraterThan()", () => {
    test("With Valid Data", () => {
        ...
        for (const { value, argument } of values) {
            expect(() => assert.number(value).toBeGraterThan(argument)).not.toThrow();
        }
    });
    ...
    test("With Invalid Data", () => {
        ...
        for (const { value, argument } of values) {
            expect(() => assert.number(value).toBeGraterThan(argument)).toThrow();
        }
    });
});
